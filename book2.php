...
if(empty($_SESSION['captcha'] ) ||
	...
	if(empty($errors))
	{
			...
			while($row = mysqli_fetch_assoc($result)) {
				...
				for ($i = $start_epoch; $i <= $end_epoch; $i=$i+600) {
					if ($i>($row["start_day"]+$row["start_time"]) && $i<($row["end_day"]+$row["end_time"])) {
						echo '<h3><font color="red">Niestety ' . $item . ' jest już zajęty.</font></h3>';
						goto end;
					}
				}
			}				
		...
		$sql = "INSERT INTO $tablename (name,login, haslo, phone, item, start_day, start_time, end_day, end_time, canceled)
			VALUES ('$name','$login','$haslo','$phone', '$item', $start_day, $start_time, $end_day, $end_time, 0)";
		if (mysqli_query($conn, $sql)) {
		    echo "<h3>Rezerwacja się powiodła i czeka na potwierdzenie przez pracownika!!!</h3>";
		} else {
...
<a href="index.php"><p>Powrot do strony głownej</p></a>
