...
<title>Dokonaj rezerwacji</title>
...
<table border="1" cellpadding="5" width="800">
	<tr>
		...
		<form action="book2.php" method="post">
			...
			<title>Każda rezerwacja musi zostać zatwierdzona przez pracownika !!!</title>
			<p>
			<input checked="checked" name="item" type="radio" value="Naprawa auta" />Naprawa auta 
			| <input name="item" type="radio" value="Przegląd" />Przegląd
			</p>
			<table style="width: 70%">
			<tr>
					<td>Login:</td>
					<td> <input maxlength="50" name="login" required="" type="text" /></td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
				</tr>
				<tr>
					<td>Hasło:</td>
					<td> <input maxlength="50" name="haslo" required="" type="password" /></td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
				</tr>
				<tr>
			...
			<input name="book" type="submit" value="Zarejestruj" />
		</form>
		</td>
		<!--<td valign="top">
		<h3>Odwołanie rezerwacji</h3>
		<form action="cancel2.php" method="post">
			...
			<p><input name="cancel" type="submit" value="Odwołaj" /></p>
		</form>
		</td>-->
	</tr>
</table>
    		...
    		if (mysqli_num_rows($result) > 0) {
    			...
    			while($row = mysqli_fetch_assoc($result)) {
    				...
    				if($current_epoch == $row["start_day"] AND $current_epoch != $row["end_day"]) {
    					$calendar .= "Rezerwacja od: " . sprintf("%02d:%02d", $row["start_time"]/60/60, ($row["start_time"]%(60*60)/60)) . "<br><hr><br>";
    				}
    				if($current_epoch == $row["start_day"] AND $current_epoch == $row["end_day"]) {
    					$calendar .= "Rezerwacja od: " . sprintf("%02d:%02d", $row["start_time"]/60/60, ($row["start_time"]%(60*60)/60)) . "<br>";
    				}
    				if($current_epoch == $row["end_day"]) {
    					$calendar .= "Rezerwacja do: " . sprintf("%02d:%02d", $row["end_time"]/60/60, ($row["end_time"]%(60*60)/60)) . "<br><hr><br>";
    				}
    				if($current_epoch != $row["start_day"] AND $current_epoch != $row["end_day"]) {
	    				$calendar .= "Rezerwacja: 24h<br><hr><br>";
	    			}
					if($row["canceled"] == 1) $calendar .= "</s></font>";
    			}
			} else {
    			$calendar .= "Brak rezerwacji";
			}
